...
const AdminDashboardScreen = ({ navigation }) => {
  ...
  const handleLogout = async () => { await logout(); navigationRef.reset({ index: 0, routes: [{ name: 'Login' }] }); };
...
const styles = StyleSheet.create({
  ...
  role: { fontSize: 14, color: COLORS.teal, fontWeight: '600', marginBottom: 16 },
  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  statCard: {
    width: '48%', backgroundColor: COLORS.cardBg, borderRadius: 12, padding: 14,
    borderLeftWidth: 4, borderWidth: 1, borderColor: COLORS.cardBorder,
  },
  statIcon: { fontSize: 20, marginBottom: 4 },
  statValue: { fontSize: 28, fontWeight: '900', color: COLORS.textPrimary },
  statLabel: { fontSize: 12, color: COLORS.textSecondary, marginTop: 2 },
  disbursedCard: { backgroundColor: '#151E30', marginTop: 8, borderColor: 'rgba(74,237,196,0.15)' },
